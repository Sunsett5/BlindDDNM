...
class DeblurringArbitral2D(A_functions):
    ...
    def Lambda(self, vec, a, sigma_y, sigma_t, eta):
        """
        Fourier-domain diagonal operator equivalent to original Lambda.
        vec: shape (B, C*H*W) (same as inputs to your A/At methods)
        Returns: same shape as vec (B, C*H*W)
        """
        device = vec.device
        bsz = vec.shape[0]

        # singulars: shape (B, M) where M corresponds to channels * num_freqs_per_channel
        singulars = self.singulars()                  # (B, M)
        M = singulars.shape[1]

        # build inverse singulars safely
        inv_s = torch.zeros_like(singulars, device=device)
        nonzero_mask = (singulars != 0)
        inv_s[nonzero_mask] = 1.0 / singulars[nonzero_mask]

        # default lambda_t = 1 for all frequencies
        lambda_t = torch.ones_like(singulars, device=device)  # (B, M)

        # treat scalars as tensors for broadcasting
        a_t = torch.tensor(a, device=device, dtype=singulars.dtype)
        sigma_y_t = torch.tensor(sigma_y, device=device, dtype=singulars.dtype)
        sigma_t_t = torch.tensor(sigma_t, device=device, dtype=singulars.dtype)
        eta_t = torch.tensor(eta, device=device, dtype=singulars.dtype)

        if (a != 0) and (sigma_y != 0):
            # condition mask: sigma_t < a * sigma_y * inv_s
            # note: inv_s has zeros where singulars==0 so mask will be False there
            change_index = (sigma_t_t < (a_t * sigma_y_t) * inv_s).float()  # (B, M) broadcast
            # new value: singulars * sigma_t * sqrt(1-eta^2) / (a * sigma_y)
            numer = singulars * sigma_t_t * torch.sqrt(1.0 - eta_t**2)
            denom = (a_t * sigma_y_t)
            replacement = numer / denom
            lambda_t = (1.0 - change_index) + change_index * replacement

        # Now map input to freq domain, scale first M entries by lambda_t, then map back
        vec_fft = self.Vt(vec)   # shape (B, total_freq_len) as produced by your Vt

        # create full-length lambda matching vec_fft length: default 1 outside first M entries
        full_len = vec_fft.shape[1]
        if full_len == M:
            lambda_full = lambda_t
        else:
            lambda_full = torch.ones((bsz, full_len), device=device, dtype=vec_fft.dtype)
            lambda_full[:, :M] = lambda_t

        out_fft = vec_fft * lambda_full
        out = self.V(out_fft)
    ...
    def Lambda_noise(self, vec, a, sigma_y, sigma_t, eta, epsilon):
        """
        Fourier-domain operator that scales the latent (vec) and noise (epsilon) according to the paper.
        vec, epsilon: shape (B, C*H*W)
        Returns: out_vec + out_eps (same shape)
        """
        device = vec.device
        bsz = vec.shape[0]
        ...
        # default d1_t and d2_t
        d1_t = sigma_t_t * eta_t * torch.ones_like(singulars, device=device)
        d2_t = sigma_t_t * torch.sqrt(1.0 - eta_t**2) * torch.ones_like(singulars, device=device)

        if (a != 0) and (sigma_y != 0):
            # Case 1: sigma_t < a * sigma_y * inv_s
            mask_small = (sigma_t_t < (a_t * sigma_y_t) * inv_s).float()
            d1_t = d1_t * (1.0 - mask_small) + mask_small * (sigma_t_t * eta_t)
            d2_t = d2_t * (1.0 - mask_small)  # becomes zero where mask_small == 1

            # Case 2: sigma_t > a * sigma_y * inv_s
            mask_large = (sigma_t_t > (a_t * sigma_y_t) * inv_s).float()
            # sqrt(sigma_t^2 - (a*sigma_y)^2 * inv_s^2)
            safe_term = (sigma_t_t**2 - (a_t * sigma_y_t)**2 * (inv_s**2)).clamp(min=0.0)
            replacement_large = torch.sqrt(safe_term)
            d1_t = d1_t * (1.0 - mask_large) + mask_large * replacement_large
            d2_t = d2_t * (1.0 - mask_large)

            # Case 3: singulars == 0 -> fall back to default behaviour
            mask_zero = (singulars == 0).float()
            d1_t = d1_t * (1.0 - mask_zero) + mask_zero * (sigma_t_t * eta_t)
            d2_t = d2_t * (1.0 - mask_zero) + mask_zero * (sigma_t_t * torch.sqrt(1.0 - eta_t**2))

        # push vec and epsilon to Fourier domain
        vec_fft = self.Vt(vec)        # (B, full_len)
        eps_fft = self.Vt(epsilon)    # (B, full_len)

        full_len = vec_fft.shape[1]
        # create full-length d1,d2 (ones outside first M entries so multiplication is identity)
        if full_len == M:
            d1_full = d1_t
            d2_full = d2_t
        else:
            d1_full = torch.ones((bsz, full_len), device=device, dtype=vec_fft.dtype)
            d2_full = torch.ones((bsz, full_len), device=device, dtype=vec_fft.dtype)
            d1_full[:, :M] = d1_t
            d2_full[:, :M] = d2_t

        out_fft = vec_fft * d1_full + eps_fft * d2_full
        out = self.V(out_fft)
        return out.reshape(vec.shape[0], -1)
...
class Deblurring(A_functions):
    ...
    def Lambda(self, vec, a, sigma_y, sigma_t, eta):
        ...
        B, C, H, W = temp_vec.shape
        temp_vec = (temp_vec * lambda_t.view(1, 1, H, W)).reshape(B, -1)

        out = self.mat_by_img(self.V_small, temp_vec).reshape(B, -1)
        return out
