...
class A_functions:
    ...
    def A_pinv(self, vec):
        ...
        eps = 1e-12
        safe_inv = torch.where(singulars > eps, 1.0 / singulars, 0.0)
        temp[:, :safe_inv.shape[0]] *= safe_inv
        return self.V(self.add_zeros(temp))

    def A_scaled_inv(self, vec, scale):
        # for PiGDM
    ...
    def A_dmps_guidance(self, vec, y, at, sigma_y):
        # for DMPS
        temp = y - 1/at.sqrt() * self.A(vec)
        temp = self.Ut(temp)
        singulars = self.singulars()
